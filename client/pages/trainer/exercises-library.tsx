    ...
    try {
      ...
      const dataToSend = {
        ...
        aliments: selectedFood.map((item) => ({
          idAliment: item.id,
          portion: parseInt(
            (document.getElementById(`portion-${item.id}`) as HTMLSelectElement)
              .value
          ),
          time: (
            document.getElementById(`time-${item.id}`) as HTMLSelectElement
          ).value
        }))
      };
      await axios
        .post(`${process.env.NEXT_PUBLIC_API_URL}/trainers/plan`, dataToSend, {
      ...
      console.error(error);
      window.alert("Error creating plan");
    }
  };
  ...
  const fetchUser = async (key: string) => {
    try {
      const users = await axios(
        `${process.env.NEXT_PUBLIC_API_URL}/trainers/listTrainees`,
        { headers: { "x-access-token": key } }
      );
      ...
      window.alert("Users loaded successfully!");
    } catch (error) {
      console.log(error);
      ...
      <div className="h-auto pt-[95px]">
        {rndExercises.length >= 1 ? (
          <div className=" flex justify-around  py-8 backdrop-blur-md bg-black bg-opacity-50 border  border-white border-l-0 border-r-0">
            <div className="flex">
              <div className=" px-4">
                <input
                  ...
                  className="w-[150px] bg-transparent text-white border px-1 border-white active:outline-none active:ring-0 focus:w-[180px] duration-[150ms]"
                />
                <button
                  onClick={handleSearch}
                  className="border border-l-0 px-2 border-white hover:text-amber-500"
                >
                  Buscar
                </button>
              </div>
              <label className="px-2" htmlFor="option-select">
                Select a filter:
              </label>
              <select
                className="pl-0 border bg-black bg-opacity-50 text-white"
                id="option-select"
                value={selectedOption}
                onChange={handleOptionChange}
              >
                <option className="border bg-black bg-opacity-50" value="">
                ...
                {options.map((option) => (
                  <option
                    key={option.value}
                    value={option.value}
                    className="backdrop-blur-md border bg-black bg-opacity-50"
                  >
                    {option.label}
                  </option>
                ))}
            ...
            {showSubOptions && (
              <div>
                <label className="px-2" htmlFor="sub-option-select">
                  Select a sub-option:
                </label>
                <select
                  className="pl-0 border bg-black bg-opacity-50 text-white"
          ...
          <div className=" flex justify-around py-8 backdrop-blur-md bg-black bg-opacity-50 border border-white border-l-0 border-r-0">
            <div className=" px-4">
              <input
                ...
                onChange={(e) => setSearchValue(e.target.value)}
                placeholder="Search by name..."
                className="w-[150px] bg-transparent text-white border px-1 border-white focus:w-[180px] duration-[150ms]"
              />
              <button
                onClick={handleSearch}
                className="border border-l-0 px-2 border-white hover:text-amber-500"
        ...
        <div className="flex flex-col gap-4 justify-around w-[80%] mx-[10%] mt-6">
          ...
          <div className="backdrop-blur-md bg-black bg-opacity-50 border  border-white h-[56px] my-auto text-center px-2">
            {" "}
            <p className="text-center py-3">
              ...
              {selectedDate ? (
                ...
                "Desbes ingresar la fecha a realizar la actividad" ? (
            ...
            <div className="flex flex-wrap border justify-center gap-1">
              {selectedExercises.length >= 1 ? (
                selectedExercises.map((ex) => (
                  <li className="min-w-min px-5 py-1" key={ex.id}>
                    Name: '{ex.name}'
                    <div>
                      <label className="px-1" htmlFor={`repeticiones-${ex.id}`}>
                        Repeticiones:
                      </label>
                      <select
                        className="pl-0 border bg-black bg-opacity-50 text-white"
                        name={`repeticiones-${ex.id}`}
                        id={`repeticiones-${ex.id}`}
                      >
                        <option
                          className="backdrop-blur-md border bg-black bg-opacity-50"
                          value="1"
                    ...
                    <div>
                      <label className="px-1" htmlFor={`series-${ex.id}`}>
                        Series:
                      </label>
                      <select
                        className="pl-0 border bg-black bg-opacity-50 text-white"
              ...
              ) : error == "Debes ingresar almenos una actividad" ? (
                <p className="py-1 text-red-500">PLEASE ADD AN ACTIVITY</p>
              ) : null}
            </div>
          </ul>
          <ul className="backdrop-blur-md  bg-black bg-opacity-50 border border-white h-auto  overflow-hidden">
            ...
            <div className="flex flex-wrap border justify-center gap-4">
                ...
                selectedFood.map((item) => {
                  return (
                    <li id={`${item.id}`} className="min-w-min px-5 py-1">
                      ...
                      <div>
                        <label className="px-1" htmlFor={`portion-${item.id}`}>
                          Portions:
                        </label>
                        <select
                          className="pl-0 border bg-black bg-opacity-50 text-white"
                        ...
                        <label className="px-1" htmlFor={`time-${item.id}`}>
                          Time:
                        </label>
                        <select
                          className="pl-0 border bg-black bg-opacity-50 text-white"
              ...
              ) : error == "Debes ingresar almenos un alimento" ? (
                <p className="py-1 text-red-500">PLEASE ADD AN ALIMENT</p>
              ) : null}
            </div>
          </ul>
        </div>
        ...
        <div className="flex mt-4">
          <button
            className="mx-auto text-center border border-white bg-black bg-opacity-50 backdrop-blur-md px-2 py-1 hover:text-amber-500"
            onClick={() => (
              setIsAlimentEndpoint(!isAlimentEndpoint),
              setFirstParam(""),
              setSelectedOption(""),
              setSelectedSubOption(""),
              setSearchValue(""),
          ...
          <label className="px-2" htmlFor={`trainee`}>
            Select a Trainee:
          </label>
          <select
            className="pl-0 border bg-black bg-opacity-50 text-white"
          ...
          >
            ...
            {selectedUsers.length >= 1 ? null : error ==
              "Trainee no encontrado" ? (
              <p className="text-red-500">PLEASE SELECT A TRAINEE</p>
            ) : null}
          </select>
          <button
            onClick={handleClick}
        ...
        <div className="pt-8">
          <div className="flex justify-around  py-5 backdrop-blur-md bg-black bg-opacity-50 border  border-white border-l-0 border-r-0">
            <p className="mx-auto text-center px-2 py-1">
              {isAlimentEndpoint
                ? "You are viewing the foods"
                : "You are viewing the exercises"}
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center w-full py-10 px-96">
          ...
          <div className="font-bold w-[35px] border border-white rounded-full py-1 backdrop-blur-lg bg-black bg-opacity-50 text-center hover:text-amber-500">
            {currentPage}
          </div>
          <button
            onClick={nextPage}
              ...
              : data?.map((item) => {
                  return (
                    ...
                    >
                      ...
                      <button
                        className="hover:text-cyan-400 border px-2 "
        ...
        <div className="flex justify-between items-center w-full py-10 px-96">
          ...
          <div className="font-bold w-[35px] border border-white rounded-full py-2 backdrop-blur-lg bg-black bg-opacity-50 text-center hover:text-amber-500">
            {currentPage}
          </div>
          <button
            onClick={nextPage}
